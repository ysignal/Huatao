...
class SSWebViewController: SSViewController {
    
    var webType: WebDataType = .url
    var linkUrl: String?
    ...
    open override func viewDidLoad() {
        super.viewDidLoad()
        fakeNav.titleLabel.text = self.title
        buildWebView()
        buildProgressView()
    ...
    open func loadPage() {
        ...
        } else if webType == .path {
            if let path = linkUrl, !path.isEmpty {
        ...
        } else if webType == .content, let htmlString = linkUrl {
            webView?.loadHTMLString(htmlString.encodeHtmlString(), baseURL: nil)
        }
    }
    ...
    private func buildProgressView() {
        ...
        progressView = UIProgressView.init(frame: CGRect.init(x: 0, y: SS.statusWithNavBarHeight, width: SS.w, height: 3))
